...
const caughtShiniesTextSingular = document.getElementById(
    "caughtShiniesTextSingular",
);
const caughtShiniesTextPlural = document.getElementById(
    "caughtShiniesTextPlural",
);
const missedShiniesCounter = document.getElementById("missedShiniesNumber");
const missedShiniesTextSingular = document.getElementById(
    "missedShiniesTextSingular",
);
const missedShiniesTextPlural = document.getElementById(
    "missedShiniesTextPlural",
);
...
function updateCaughtShinies() {
    ...
    if (caughtShinies === 1) caughtShiniesTextSingular.style.display = "block";
    if (caughtShinies > 1) {
...
function updateMissedShinies() {
    ...
    if (missedShinies === 1) missedShiniesTextSingular.style.display = "block";
    if (missedShinies > 1) {
...
function animateInputFeedback(color) {
    input.animate(
        { backgroundColor: [color], offset: 0.2 },
        { duration: 1e3, iterations: 1, easing: "ease-in-out" },
    );
}
...
function getBoundaries() {
    ...
    maximumId = clamp(minimumId, parseInt(maximumInput.value) || 1025, 1025);
...
function setBufferSize() {
    ...
    if (range < 2) bufferSize = 0;
    else if (range === 2 || range === 3) bufferSize = 1;
    else if (range === 4 || range === 5) bufferSize = 2;
    else {
    ...
    switch (gameMode) {
        case "nameToNumber":
            ...
            pokemonSprite.onload = () => {
                if (isShiny && shinyShakeActive) shinyShake();
            };
...
function getLanguage() {
    ...
    const isSupported = ["en", "fr", "es", "it", "de", "ja"].includes(
        preferredLanguage,
    );
...
function updateInputPlaceholderText() {
    switch (gameMode) {
        case "nameToNumber":
            input.setAttribute(
                "placeholder",
                localisation["inputPlaceholderNumber"][language],
            );
            break;
        case "numberToName":
            input.setAttribute(
                "placeholder",
                localisation["inputPlaceholderName"][language],
            );
...
restartBtn.addEventListener("click", () => {
    if (optionChange) applyOptionsChanges();
    updateRestartButtonDisplay();
    resetScore();
    initializeBuffer();
    newRound();
...
document.querySelectorAll(".flag").forEach((flag) =>
    flag.addEventListener("click", () => {
        ...
        document
            ...
            .forEach((flag) => flag.classList.remove("flagFocus"));
...
document.querySelectorAll("input[name=mode]").forEach((btn) =>
...
document.querySelectorAll(".boundaryInput").forEach((element) =>
